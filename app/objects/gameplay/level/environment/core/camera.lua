--
-- Created by Victor on 7/9/2015 9:46 AM
--
...
function camera:prepare(player)
    ...
    self.tolerance_ = 16
    self:getPhysicsBody():getFirstShape():setTag(cc.tags.camera)
...
function camera:step(dt)
    ...
    if self.scroll_collision_ ~= nil and self.camera_mode_ ~= cc.CAMERA.MODE.SHIFT then
        ...
        local camera_shape_size = self:getPhysicsBody():getFirstShape().size_
        local scroll_shape_size = self.scroll_collision_:getPhysicsBody():getFirstShape().size_
