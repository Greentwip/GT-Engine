--
-- Created by Victor on 7/7/2015 5:30 AM
--
...
function level_base:offset_position(position, enemy, tmx_element) -- used for enemies, to acquire center
    ...
    local body_offset = enemy:getPhysicsBody():getFirstShape():getCenter()
    local y_offset = body_offset.y
    y_offset = y_offset + tmx_element.height * 0.5

    position.y = position.y + enemy:getPhysicsBody():getFirstShape().size_.height * 0.5 - y_offset
    return position
end
...
function level_base:load(tmx_map, load_arguments)
    ...
    for i = 1, #built_logical do
       if built_logical[i]:getPhysicsBody():getFirstShape():getTag() == cc.tags.check_point then
    ...
    camera:setPosition(0, camera:getPhysicsBody():getFirstShape().size_.height * 0.25)
    ...
    if enemy_group ~= nil then
        ...
        for  i = 1, #group_array do
            ...
            enemy:init(self:offset_position(self:calculate_tmx_position(group_array[i], map), enemy, group_array[i]), player)
